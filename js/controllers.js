...
// show students
// var customUrl = 'http://localhost/academy-1/api/public/';
// var customUrl = 'http://maths.bikayacademy.org/api/public/';
var customUrl = 'http://188.166.234.165/maths/api/public/';

function studentsCtrl($scope,$http,$timeout,$filter){
    $scope.myDate = new Date();
    $scope.getData = function(){
        $http.get(customUrl+'students')
    ...
    $scope.create = function() {
        $http.post(customUrl+'studentpostData',{data:$scope.newStudent})
            .success(function (data){
                // debugger
    ...
    $scope.editStudent=function (student) {
      ...
      if (!!$scope.newStudent.sid) {
            // console.log("aaa"+$scope.newStudent.sid);
            $scope.buttonName = "Update";
      }
      else{
            // console.log("bbb"+$scope.newStudent.sid);
            $scope.buttonName = "create";
      }
      // $scope.isForm = true;
      // $scope.isEdit = true;
      // $scope.isNew = false;
      // $scope.pageTitle = 'Edit Student';
    }
    ...
    $scope.updateStudent = function(){
        $http({
            method: 'PUT',
            url: customUrl+'editStudent/' + $scope.newStudent.sid,
            data: $scope.newStudent
        }).success(function (data) {
            // debugger
            //find id of old movie
            student = $filter('filter')($scope.students, {id: data.sid})[0];
            //replace old movie by new
            index = $scope.students.indexOf(student);
            $scope.students[index]= data;
            //$scope.newMovie = "";
          })
        .error(function (response, status) {
            $log.warn(response);
    ...
    $scope.deleteStudent = function(student) {
          $http({
            method: 'DELETE',
            url: customUrl+'deleteStudent/' +student.sid
          }).success(function (data) {
...
function exerciseCtrl($scope,$http,$timeout,$filter, $location,$stateParams,$state){
   
    $scope.getEx = function(){
        $http.get(customUrl+'exercise')
            .success(function (data){
                // console.log(data);
                $scope.exercises = data;
                
                // exercise = $filter('filter')($scope.exercises, {id: data.id})[0];
    ...
    $scope.detail = function(exercise){
        $scope.newExercise = exercise;       
        $location.path('#/exercise/'+exercise.id);
    }
    // $scope.detail();
    ...
    $scope.getStudent = function(id){
        ...
        // console.log(exercise.id);
         $http.get(customUrl+'getchapter/'+$stateParams.id)
            .success(function (data){               
                $scope.chapters = data;
            })
            .error(function (response,status){
